...
The unit panorama also includes a third business-catalog basemap after business tree and business matrix. Keep six switchable business mainlines in a directory-style left rail; each mainline carries 6–8 horizontally scrollable business blocks, each block contains 4–7 vertically scrollable business units, and each card combines one business unit with its business-item list. Do not force every mainline to exactly four blocks or every block to exactly four units.

Keep the business-catalog basemap as large as the surrounding layout allows, with minimal top and bottom margins. Use one shared scrolling canvas for all business blocks and units, with horizontal and vertical scrolling on the overall block canvas; do not add independent vertical scrollbars inside each business block. Unit cards must grow or reserve enough height to contain every business-item row without overflow. The shared horizontal scrollbar should sit close to the bottom edge rather than floating noticeably above it.
